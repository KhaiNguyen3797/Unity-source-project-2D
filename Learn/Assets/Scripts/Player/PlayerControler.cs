...
public class PlayerControler : MonoBehaviour
{
        ...
        if (!ground)
        {
            ...
            for (int i = 0; i < collider2s.Length; i++)
            {
                if (collider2s[i].gameObject != gameObject)
                {
                    if (land)
                    {
                        GameObject smoke = Instantiate(smokeEffect, checkGround.position, Quaternion.identity);
                        Destroy(smoke, 3);
                    }
                    ground = true;
                    land = false;
    ...
    public void Move(bool jump, bool runCircle, float move)
    {
        ...
        if (ground && jump)
        {
            GameObject jumpSmoke = Instantiate(smokeEffect, checkGround.position, Quaternion.identity);
            Destroy(jumpSmoke, 3f);
            jump = false;
            checkJump = true;
